...
function makeCommandLine(options, videoCodec, audioCodec) {
  ...
  if (options.audioInterface && options.audioDelay > 0)
    cmdline = cmdline.concat(`-af "adelay=delays=${options.audioDelay}:all=1"`.split(" ")); //Apply an audio filter to delay all audio channels for x ms
