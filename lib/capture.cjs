...
const { getDefaultProfile, ffmpeg, makeCommandLine } = require("./ffmpeg.cjs");
...
async function hwencode(filePath, codec, option = {}) {
  ...
  if(!["h264_nvenc","h264_amf","hevc_nvenc","hevc_amf"].includes(codec)) throw "ERR_UNKNOWN_CODEC";
  ...
  const options = {
    ...
    videoEncodingOptions: option.videoEncodingOptions || getDefaultProfile(codec, option.bits10),
