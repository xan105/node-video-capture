...
function getDefaultProfile(codec, bits10 = false){
  ...
  const profiles = {
    h264_nvenc: "-b:v 6000k -minrate:v 3000k -maxrate:v 9000k -bufsize:v 9000k -rc:v vbr -level:v 4.2 -g:v 120 -bf:v 3 -qp:v 19", //Tested with GTX 1060 | Target quality is 15mb-ish for 20s clip
    h264_amf: "-b:v 6000k -minrate:v 3000k -maxrate:v 9000k -bufsize:v 9000k -rc:v cqp -level:v 4.2 -g:v 120 -bf:v 3", //Reported working with RX 580
...
function makeCommandLine(options, videoCodec, audioCodec) {
  ...
  //Output encoding
  const videoEncodingOptions = options.videoEncodingOptions || getDefaultProfile(videoCodec[1], options.bits10);
